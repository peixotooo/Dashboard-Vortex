...
export async function GET(
  ...
  let selection;
  try {
    selection = await selectGroupForPoolRedirect(admin, slug);
  } catch (error) {
    console.error("[whatsapp-group-pool] failed to select redirect group", error);
    return htmlResponse(
      "Grupo indisponivel",
  ...
  try {
    ...
    console.warn("[whatsapp-group-pool] failed to record redirect", error);
  }
