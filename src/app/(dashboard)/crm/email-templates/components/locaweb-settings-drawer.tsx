...
export function LocawebSettingsDrawer({ workspaceId }: { workspaceId: string }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/crm/email-templates/locaweb/settings", {
      ...
      .then((d: Settings) => {
        ...
        // Don't pre-fill the token; show only the redacted form.
      });
  ...
  const save = async () => {
    ...
    try {
      ...
      // Only send the token if the user actually typed something — empty
      // input means "keep the existing value" (so the redacted form doesn't
      // wipe it).
        ...
        <div className="space-y-5 py-2">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Credenciais e remetente padrão pra disparar campanhas via Locaweb.
            </p>
          </div>
          ...
          {settings === null ? (
            ...
            <>
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    placeholder={settings.token_set ? "(deixe vazio pra manter o atual)" : "Cole o token da Locaweb"}
                    className="h-9 text-sm font-mono"
                  />
                </div>
              ...
              <div className="text-[10px] text-muted-foreground leading-relaxed border-t pt-3">
                Sem webhooks na Locaweb. Stats (open/click/bounce) são
                puxadas a cada 6h por um cron e aparecem no histórico de
                cada draft disparado.
              </div>
            </>
          )}
        </div>
