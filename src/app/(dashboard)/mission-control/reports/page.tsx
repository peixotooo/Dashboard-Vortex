...
import type { ExecutiveReport } from "@/lib/mission-control/types";
...
export default function ReportsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/mission-control/reports", {
  ...
  const save = async () => {
    ...
    const path = editing.id
      ? `/api/mission-control/reports/${editing.id}`
      : "/api/mission-control/reports";
    await fetch(path, {
      method,
      headers: {
  ...
  const markSent = async (r: ExecutiveReport) => {
    if (!workspace?.id) return;
    await fetch(`/api/mission-control/reports/${r.id}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        "x-workspace-id": workspace.id,
