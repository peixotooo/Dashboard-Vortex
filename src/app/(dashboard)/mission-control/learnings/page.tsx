...
import type { Decision, Learning } from "@/lib/mission-control/types";
import { formatShortDateTime } from "@/lib/mission-control/format";
...
export default function LearningsDecisionsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [lRes, dRes] = await Promise.all([
        fetch("/api/mission-control/learnings", {
          headers: { "x-workspace-id": workspace.id },
        }),
        fetch("/api/mission-control/decisions", {
          headers: { "x-workspace-id": workspace.id },
        }),
      ]);
      if (lRes.ok) {
  ...
  const saveLearning = async () => {
    ...
    const path = editingLearning.id
      ? `/api/mission-control/learnings/${editingLearning.id}`
      : "/api/mission-control/learnings";
    await fetch(path, {
      method,
      headers: {
  ...
  const saveDecision = async () => {
    ...
    const path = editingDecision.id
      ? `/api/mission-control/decisions/${editingDecision.id}`
      : "/api/mission-control/decisions";
    await fetch(path, {
      method,
      headers: {
        "Content-Type": "application/json",
