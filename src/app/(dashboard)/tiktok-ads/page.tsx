  ...
  const fetchCampaigns = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (data.needs_connection) setNeedsConnection(true);
            ...
            {needsConnection ? (
              <div className="mt-3">
                <Button asChild size="sm">
                  <a href={`/api/tiktok/auth?workspace_id=${workspace?.id || ""}`}>
                    Conectar TikTok
                  </a>
                </Button>
              </div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Verifique se o app TikTok foi aprovado para producao e se o scope de
                Reporting esta liberado. Em sandbox, use uma Sandbox Ad Account.
              </p>
            )}
          </CardContent>
        </Card>
      ...
      {!loading && !error && sorted.length === 0 && (
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              Nenhuma campanha TikTok Ads encontrada
            </p>
          </CardContent>
        </Card>
      )}
