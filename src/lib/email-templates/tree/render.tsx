...
function buildEditorScript(mode: Mode): string {
  ...
  return `(function(){var sel=null;function clear(){if(sel){sel.style.outline='';sel.style.outlineOffset='';sel=null;}}document.addEventListener('mouseover',function(e){var t=e.target&&e.target.closest&&e.target.closest('[data-block-id]');if(!t||t===sel)return;var prev=document.querySelector('.__hover');if(prev){prev.classList.remove('__hover');prev.style.outline='';}t.classList.add('__hover');t.style.outline='${hover}';t.style.outlineOffset='-1px';},true);document.addEventListener('mouseout',function(e){var t=e.target&&e.target.closest&&e.target.closest('[data-block-id]');if(!t||t===sel)return;t.classList.remove('__hover');t.style.outline='';},true);document.addEventListener('click',function(e){var t=e.target&&e.target.closest&&e.target.closest('[data-block-id]');if(!t)return;e.preventDefault();e.stopPropagation();clear();sel=t;t.style.outline='${selected}';t.style.outlineOffset='-2px';try{parent.postMessage({type:'block:select',id:t.getAttribute('data-block-id')},'*');}catch(err){}},true);window.addEventListener('message',function(e){if(e.source!==parent||!e.data||e.data.type!=='block:set-selected')return;clear();var id=e.data.id;if(typeof id!=='string'||!/^[\\w.-]{1,128}$/.test(id))return;var nodes=Array.prototype.filter.call(document.querySelectorAll('[data-block-id]'),function(n){return n.getAttribute('data-block-id')===id;});if(nodes.length===0)return;sel=nodes[0];nodes.forEach(function(n){n.style.outline='${selected}';n.style.outlineOffset='-2px';});});})();`;
}
